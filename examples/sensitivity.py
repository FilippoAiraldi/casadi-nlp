...
nlp.minimize((1 - x[0])**2 + a * (x[1] - x[0]**2)**2)
g = (x[0] + 0.5)**2 + x[1]**2
nlp.constraint('c1', (p / 2)**2, '<=', g)
_, lam = nlp.constraint('c2', g, '<=', p**2)
...
ax.plot(pv.flat, Z(pv, 0).full().flat, 'k.', markersize=8)
