    ...
    def test_h_lbx_ubx__returns_correct_indices(self):
        for flag in (True, False):
            ...
            if flag:
                ...
                np.testing.assert_allclose(
                    cs.evalf(h_lbx - cs.vertcat(-np.inf - x1, 0 - x2)), 0
                )
                np.testing.assert_allclose(
                    cs.evalf(lam_lbx - cs.vertcat(lam_lbx1, lam_lbx2)), 0
                )
                np.testing.assert_allclose(
                    cs.evalf(h_ubx - cs.vertcat(x1 - np.inf, x2 - np.inf)), 0
                )
                np.testing.assert_allclose(
                    cs.evalf(lam_ubx - cs.vertcat(lam_ubx1, lam_ubx2)), 0
                )
            ...
            if flag:
                ...
                np.testing.assert_allclose(
                    cs.evalf(h_lbx - cs.vertcat(-np.inf - x1, 0 - x2, -np.inf - x3)), 0
                )
                np.testing.assert_allclose(
                    cs.evalf(lam_lbx - cs.vertcat(lam_lbx1, lam_lbx2, lam_lbx3)), 0
                )
                np.testing.assert_allclose(
                    cs.evalf(h_ubx - cs.vertcat(x1 - np.inf, x2 - np.inf, x3 - 1)), 0
                )
                np.testing.assert_allclose(
                    cs.evalf(lam_ubx - cs.vertcat(lam_ubx1, lam_ubx2, lam_ubx3)), 0
                )

            x4, lam_lbx4, lam_ubx4 = nlp.variable("x4", (2, 1), lb=0, ub=1)
            (h_lbx, lam_lbx), (h_ubx, lam_ubx) = nlp.h_lbx, nlp.h_ubx
            if flag:
                np.testing.assert_allclose(cs.evalf(h_lbx - cs.vertcat(-x2, -x4)), 0)
                np.testing.assert_allclose(
                    cs.evalf(lam_lbx - cs.vertcat(lam_lbx2, lam_lbx4)), 0
                )
                np.testing.assert_allclose(
                    cs.evalf(h_ubx - cs.vertcat(x3 - 1, x4 - 1)), 0
                )
                np.testing.assert_allclose(
