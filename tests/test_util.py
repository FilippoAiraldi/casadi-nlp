...
class DummyWithCachedProperty:
    ...
    @tools.cache_clearer(prop1, prop2)
...
class TestTools(unittest.TestCase):
    ...
    def test_np_random__initializes_rng_with_correct_seed(self):
        for seed in (69, None):
            rng, actual_seed = tools.np_random(seed)
...
class TestCasadi(unittest.TestCase):
    def test_is_casadi_object__guesses_correctly(self):
        for o, flag in [
            ...
            self.assertEqual(casadi.is_casadi_object(o), flag)
    ...
    def test_dict2struct__with_DM__returns_numerical_struct(self):
        ...
        s = casadi.dict2struct(d)
        self.assertIsInstance(s, casadi.DMStruct)
    ...
    def test_dict2struct__with_SX__returns_sym_struct(self):
        ...
        s = casadi.dict2struct(d)
        self.assertIsInstance(s, casadi.struct_symSX)
        for name, x in d.items():
...
class TestMath(unittest.TestCase):
    ...
    def test_quad_form(self):
        ...
        for m in (1, n):
            ...
            p_sx = subsevalf(
                math.quad_form(A_sx, x_sx), [x_sx, A_sx], [x, A])
            p_mx = subsevalf(
                math.quad_form(A_mx, x_mx), [x_mx, A_mx], [x, A])
    ...
    def test_norm_cdf(self):
        ...
        cdf_sx = subsevalf(math.norm_cdf(x_sx, loc=loc_sx, scale=scale_sx),
                           [x_sx, loc_sx, scale_sx], [x, loc, scale])
        cdf_mx = subsevalf(math.norm_cdf(x_mx, loc=loc_mx, scale=scale_mx),
    ...
    def test_norm_ppf(self):
        ...
        loc = np.random.randn(*shape)
        scale = np.random.rand(*shape)
        cdf_sx = subsevalf(math.norm_ppf(x_sx, loc=loc_sx, scale=scale_sx),
                           [x_sx, loc_sx, scale_sx], [x, loc, scale])
        cdf_mx = subsevalf(math.norm_ppf(x_mx, loc=loc_mx, scale=scale_mx),
                           [x_mx, loc_mx, scale_mx], [x, loc, scale])
