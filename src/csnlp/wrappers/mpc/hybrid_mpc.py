...
class HybridMpc(Mpc[SymType]):
    ...
    def set_pwa_dynamics(self, pwa_system: dict) -> None:
        ...
        if not all(pwa_system['T'][i].shape[0] == l for i in range(s)):
            raise ValueError("The number of inequalities defining regions must be equal for all regions.")
        if self._is_multishooting:
            self._multishooting_pwa_dynamics(pwa_system)
        else:
            self._single_shooting_pwa_dynamics(pwa_system)
        self._dynamics = pwa_system # TODO is it okay to set _dynamics as the dict? I could make a Casadi function which evaluates the if-else of the PWA dyanmics
    ...
    def _multishooting_pwa_dynamics(self, pwa_system: dict) -> None:
        ...
        M_region = np.zeros((s, l)) # big-M relaxation for region constraints (number of region, number of inequalities)
        for i in range(s):
            region = S[i] @ x + R[i] @ u - T[i]
            for j in range(l):
                # TODO add solve options including verbose
                lp = {'x': cs.vertcat(x, u), 'f': -region[j], 'g': cs.vertcat(D @ x - E, F @ u - G)}    # negative sign for maximization
        ...
        for j in range(n):
            for i in range(s):
                lp = {'x': cs.vertcat(x, u), 'f': -(A[i][[j], :]@x + B[i][[j], :]@u + c[i][[j], :]), 'g': cs.vertcat(D @ x - E, F @ u - G)} 
                solver = cs.qpsol('S', 'clp', lp)
                sol = solver(ubg=0)
                temp_upper[i] = sol['f']
        ...
        for k in range(N):
            for i in range(s):
                self.constraint(
                    f"region_k{k}_s{i}",
                    S[i]@ X[:, [k]] + R[i] @ U[:, [k]] - T[i] - M_region[i, :] @ (1 - delta[i, k]),
                    "<=", 0,
                )
                # self.constraint(
                #     f"z_ub_k{k}_s{i}",
                ...
                #     z[i][:, [k]],
                #     "<=",
                #     A[i]@X[:, [k]] + B[i]@U[:, [k]] + c[i] - M_lb @ (1 - delta[i, [k]])
                # )
                # self.constraint(
                #     f"z_x_lb_k{k}_s{i}",
                #     z[i][:, [k]],
                #     ">=",
                #     A[i]@X[:, [k]] + B[i]@U[:, [k]] + c[i] - M_ub @ (1 - delta[i, [k]])
                # )
        ...
        #     )
            ...
            # )
        #     self.constraint(
        #         f"state_constraint_k{k}",
        #         D @ X[:, [k]] - E,
        #         "<=",
        #         0
        #     )
        #     self.constraint(
        #         f"input_constraint_k{k}",
        #         F @ U[:, [k]] - G,
        #         "<=",
        #         0
        #     )
        # self.constraint(
        #         f"state_constraint_k{N}",
        #         D @ X[:, [N]] - E,
        #         "<=",
        #         0
        #     )
    ...
    def _single_shooting_pwa_dynamics(self, pwa_system: dict) -> None:
        ...
        raise NotImplementedError('Single shooting for PWA dynamics is not yet implemented.')